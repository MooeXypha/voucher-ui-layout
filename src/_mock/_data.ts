...
// voucher mock data matching CreateVoucherDto fields
export const _vouchers = [...Array(24)].map((_, index) => {
  const setIndex = index + 1;
  const phonePrefix = setIndex % 2 === 0 ? '09' : '+959';
  ...
  return {
    ...
    accountCategory: ['Personal', 'Business'][index % 2],
    accountUserName: `user${index}`,
    amountPaid: _price(index),
    prepaid: _boolean(index),
    paymentMethod: ['Cash', 'Credit Card', 'Mobile Wallet'][index % 3],
    paymentDate: _times(index),
    remark: index % 3 ? _description(index) : undefined,
  };
});
